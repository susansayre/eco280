...
You’re now ready to get your population data ready to merge with the electricity data. In Stata, you’ll need to save this dataset so you can load a new one. Do this by typing `save electricity_data, replace`. This will save your current data in a file called `electricity_data.dta`. The replace option means that the data will be overwritten when you re-run the code.
